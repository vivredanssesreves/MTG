...
const pathToMyJson = `${pathSrc}data-user/mtg/json/`;
...
export function importChunkForSet(setCode, cards) {
    const pathMyCards = `${pathToMyJson}${setCode}.json`;

    // Load existing data
    let mySetData = { cards: [] };
    if (fs.existsSync(pathToMyJson)) {
        try {
            mySetData = JSON.parse(fs.readFileSync(pathToMyJson, 'utf-8'));
        } catch (error) {
            return {
                success: false,
                imported: 0,
    ...
    // Save the file
    try {
        fs.writeFileSync(pathToMyJson, JSON.stringify(mySetData, null, 2));
    } catch (error) {
        return {
            success: false,
            imported: 0,
